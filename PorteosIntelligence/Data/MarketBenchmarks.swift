...
struct MarketBenchmarks {
    ...
    static func benchmark(for city: String) -> CityMetrics? {
        ...
        let aliases: [String: String] = [
            ...
            "münchen":         "Munich",
            "muenchen":        "Munich",
            "cologne":         "Frankfurt",     // nearest benchmark
            "köln":            "Frankfurt",
            "wien":            "Vienna",
            "vienne":          "Vienna",
            "zuerich":         "Zurich",
        ...
        return benchmarks.first { $0.cityName.lowercased().hasPrefix(query) }
            ?? benchmarks.first { $0.cityName.lowercased().contains(query) }
    }

    /// Returns up to `limit` cities whose names start with `prefix` (case-insensitive).
    /// Useful for autocomplete drop-downs.
    static func suggestions(matching prefix: String, limit: Int = 6) -> [CityMetrics] {
        let query = prefix.trimmingCharacters(in: .whitespaces).lowercased()
        guard !query.isEmpty else { return [] }

        let starts   = benchmarks.filter { $0.cityName.lowercased().hasPrefix(query) }
        let contains = benchmarks.filter { !$0.cityName.lowercased().hasPrefix(query)
                                        &&  $0.cityName.lowercased().contains(query) }
        return Array((starts + contains).prefix(limit))
    }

    /// All unique regions present in the database.
    static var regions: [String] {
        Array(Set(benchmarks.map(\.region))).sorted()
    }

    /// All cities for a given region.
